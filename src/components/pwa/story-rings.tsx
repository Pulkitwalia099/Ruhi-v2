    ...
    <>
      <div className="flex gap-4 overflow-x-auto py-4 px-1 scrollbar-hide">
        ...
        <Ring
          ...
          hasNew={false}
          onClick={() => {
            window.location.href = `/chat/${crypto.randomUUID()}?prompt=Scan%20my%20face`;
          }}
        />
      </div>
      ...
      <Sheet open={activeSheet === "score"} onOpenChange={(open) => !open && setActiveSheet(null)}>
        <SheetContent side="bottom" className="rounded-t-2xl" style={{ backgroundColor: "#1A1815", borderColor: "rgba(250, 250, 249, 0.08)" }}>
          ...
          <div className="py-4 space-y-4">
            ...
            <button
              type="button"
              onClick={() => { setActiveSheet(null); window.location.href = `/chat/${crypto.randomUUID()}?prompt=My%20progress`; }}
      ...
      <Sheet open={activeSheet === "streak"} onOpenChange={(open) => !open && setActiveSheet(null)}>
        <SheetContent side="bottom" className="rounded-t-2xl" style={{ backgroundColor: "#1A1815", borderColor: "rgba(250, 250, 249, 0.08)" }}>
          ...
          <div className="py-4 space-y-3 text-center">
            ...
            <button
              type="button"
              onClick={() => { setActiveSheet(null); window.location.href = `/chat/${crypto.randomUUID()}?prompt=Scan%20my%20face`; }}
              className="w-full text-center text-sm py-2"
              style={{ color: "#3DB9A8" }}
            >
          ...
          <div className="py-4 space-y-3">
            {cycleInfo ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => { setActiveSheet(null); window.location.href = `/chat/${crypto.randomUUID()}?prompt=What%20should%20I%20adjust%20for%20my%20cycle%20phase`; }}
                  className="w-full text-center text-sm py-2"
                  style={{ color: "#3DB9A8" }}
                >
